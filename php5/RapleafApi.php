<?php
  ...
  */
  ...
  class RapleafApi {
    ...
    private static $BULK_PATH = "https://personalize.rapleaf.com/v4/bulk?api_key=";
    ...
    function query_by_email($email, $hash_email = false, $show_available = false) {
      ...
      if ($hash_email) {
        ...
        return self::query_by_sha1($sha1_email, $show_available);
      } else {
        $url = self::$BASE_PATH . self::$API_KEY . "&email=" . urlencode($email);

        return self::get_json_response($url, $show_available);
      }
    }
    ...
    private function get_json_response($url, $show_available) {
